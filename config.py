...
unique_filename = model_name + \
                  ("" if use_web_weights else f"_size_{size}") + \
                  ("" if use_web_weights else f"_epochs_{epochs}")
trained_filepath = os.path.join(CHECKPOINT_PATH, unique_filename + ".ckpt")
results_filepath = os.path.join(RESULTS_PATH, unique_filename + ".png")
